...
import AiSchedulePage from "./AiSchedulePage";
...
const FLIGHT_DATA = {
  "EK205": {
    ...
    report: {
      id: "EK205", route: "DXB→LHR", aircraft: "B777-300ER", grade: "A+", crew: "Capt. Hassan, F/O Chen", landed: "14:08 UTC", duration: "6h 53m", nm: "3,414 NM",
  ...
  "EK412": {
    ...
    report: {
      id: "EK412", route: "DXB→JFK", aircraft: "B777-300ER", grade: "C+", crew: "Capt. Al-Farsi, F/O Johnson", landed: "IN FLIGHT", duration: "est. 14h 05m", nm: "6,837 NM",
  ...
  "EK508": {
    ...
    report: {
      id: "EK508", route: "DXB→SYD", aircraft: "A380-800", grade: "A", crew: "Capt. Williams, F/O Al-Maktoum", landed: "PRE-FLIGHT", duration: "est. 14h 35m", nm: "7,480 NM",
  ...
  "EK118": {
    ...
    report: {
      id: "EK118", route: "DXB→CDG", aircraft: "B777-200LR", grade: "A", crew: "Capt. Al-Rashidi, F/O Müller", landed: "12:14 UTC", duration: "6h 44m", nm: "3,252 NM",
    ...
    <div style={{ minHeight: "100vh", background: "#070c14", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ animation: "fadeIn .6s ease", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", width: 380 }}>
        ...
        <div style={{ width: "100%", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(0,229,192,0.15)", borderRadius: 16, padding: "36px 32px", backdropFilter: "blur(20px)" }}>
          <div style={{ fontSize: 11, color: "#555", letterSpacing: 3, marginBottom: 8, fontFamily: "'DM Mono',monospace" }}>SECURE ACCESS · OPERATOR LOGIN</div>
          <div style={{ fontSize: 9, color: "#888", letterSpacing: 1, marginBottom: 24, fontFamily: "'DM Mono',monospace" }}>Login as system 'ADMIN' for Aircraft Control</div>
          ...
          {name.trim().toUpperCase() === "ADMIN" && (
            <div style={{ marginBottom: 28, padding: "8px 12px", background: "rgba(255,59,92,0.05)", border: "1px solid rgba(255,59,92,0.15)", borderRadius: 8 }}>
              <div style={{ fontSize: 9, color: "#ff3b5c", fontFamily: "'DM Mono',monospace", letterSpacing: 1 }}>⚠️ ADMINISTRATOR ACCESS DETECTED — ENTER PASSWORD</div>
            </div>
          )}
...
function FlightSelectPage({ currentUser, flights, setPage, onSelect, onLogout, onViewHistory, onViewProfile, onViewActivityLog, onViewAdminControl }) {
    ...
    <div style={{ minHeight: "100vh", background: "#070c14", fontFamily: "'Outfit',sans-serif", color: "#e0e8f0", display: "flex", flexDirection: "column" }}>
              ...
              <>
                ...
                <button
                  ...
                  style={{
                    ...
                    letterSpacing: 1,
                    minHeight: isMobile ? 44 : "auto",
                    boxShadow: "0 0 10px rgba(255,59,92,0.2)"
                  }}
                >
                  ⚡ AI SCHEDULE MANAGER
                </button>
                <button
                  onClick={onViewAdminControl}
                  style={{
                    ...
                    letterSpacing: 1,
                    minHeight: isMobile ? 44 : "auto",
                    boxShadow: "0 0 10px rgba(0,118,255,0.2)"
                  }}
                >
                  ⚙ AIRCRAFT CONTROL
                </button>
      ...
      <div style={{ flex: 1, padding: isMobile ? "40px 24px 48px" : "52px 40px 40px", animation: "fadeIn .5s ease", zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2,1fr)", gap: isMobile ? 32 : 20, maxWidth: 1040, margin: "0 auto" }}>
          {(flights || FLIGHT_DETAILS).map((f, i) => {
...
// ─── Simulation Map Component (Replaces Globe) ─────────────────────────────────
// Custom Leaflet Icons
const planeIconHtml = `<div style="transform: rotate(var(--rotation, 0deg)); font-size: 24px; color: #fff; text-shadow: 0 0 10px rgba(0,229,192,0.8); display:flex; align-items:center; justify-content:center; width: 100%; height: 100%;">✈</div>`;
const planeIcon = new L.DivIcon({ html: planeIconHtml, className: "custom-plane-icon", iconSize: [30, 30], iconAnchor: [15, 15] });

const dotIconHtml = (color) => `<div style="width: 8px; height: 8px; background: ${color}; border-radius: 50%; box-shadow: 0 0 10px ${color}"></div>`;
const originDot = new L.DivIcon({ html: dotIconHtml("#00e5c0"), className: "", iconSize: [8, 8], iconAnchor: [4, 4] });
const destDot = new L.DivIcon({ html: dotIconHtml("#ff3b5c"), className: "", iconSize: [8, 8], iconAnchor: [4, 4] });

function SimulationGlobe({ route }) {
  const [routeAirports, setRouteAirports] = useState(["DXB", "LHR"]);

  useEffect(() => {
    if (route) {
      setRouteAirports(route.split("→").map(s => s.trim()));
    }
  }, [route]);

  // Real world coordinates for demo (Lat, Lon)
  const coords = {
    "DXB": [25.2532, 55.3657],
    "LHR": [51.4700, -0.4543],
    "JFK": [40.6413, -73.7781],
  ...
  const originCode = routeAirports[0] || "DXB";
  const destCode = routeAirports[1] || "LHR";
  const origin = coords[originCode] || coords["DXB"];
  const dest = coords[destCode] || coords["LHR"];

  // Animation state for the plane
  const [progress, setProgress] = useState(0);

  useEffect(() => {
    let animationFrameId;
    let start = Date.now();
    const duration = 12000; // 12 seconds for a full trip
  ...
  // Calculate plane position and curve (Bezier-like arc for 2D map)
  const calculateCurve = (p1, p2) => {
    const latDiff = p2[0] - p1[0];
    const lonDiff = p2[1] - p1[1];
    const midLat = (p1[0] + p2[0]) / 2 + (Math.abs(lonDiff) * 0.2); // Simple arc effect
    const midLon = (p1[1] + p2[1]) / 2;
    return [midLat, midLon];
  };

  const mid = calculateCurve(origin, dest);
  const t = progress;

  // Quadratic Bezier interpolation for current position
  const currentLat = (1 - t) * (1 - t) * origin[0] + 2 * (1 - t) * t * mid[0] + t * t * dest[0];
  const currentLon = (1 - t) * (1 - t) * origin[1] + 2 * (1 - t) * t * mid[1] + t * t * dest[1];
  const currentPos = [currentLat, currentLon];

  // Rotation angle calculation
  const dx = 2 * (1 - t) * (mid[1] - origin[1]) + 2 * t * (dest[1] - mid[1]);
  const dy = 2 * (1 - t) * (mid[0] - origin[0]) + 2 * t * (dest[0] - mid[0]);
  const angle = Math.atan2(dx, dy) * (180 / Math.PI); // Leaflet standard bearing style orientation

  // Map center logic
  const centerLat = (origin[0] + dest[0]) / 2;
  const centerLon = (origin[1] + dest[1]) / 2;

  // Generate polyline curve positions
  const polylinePositions = [];
  for (let i = 0; i <= 20; i++) {
    let pt = i / 20;
    let clat = (1 - pt) * (1 - pt) * origin[0] + 2 * (1 - pt) * pt * mid[0] + pt * pt * dest[0];
    let clon = (1 - pt) * (1 - pt) * origin[1] + 2 * (1 - pt) * pt * mid[1] + pt * pt * dest[1];
    polylinePositions.push([clat, clon]);
  }

  // Inject rotation into the custom plane icon (since React Leaflet Marker rotation is tricky)
  useEffect(() => {
    const iconElems = document.getElementsByClassName("custom-plane-icon");
    if (iconElems.length > 0) {
      iconElems[0].style.setProperty("--rotation", `${angle}deg`);
    }
  }, [angle]);

  return (
    <div style={{ position: "relative", width: "100%", height: 400, background: "#0a0a0a", borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.05)" }}>
      ...
      >
        ...
        {/* Airport Markers */}
        <Marker position={origin} icon={originDot} />
        <Marker position={dest} icon={destDot} />
      ...
      {/* Overlays */}
      <div style={{ position: "absolute", top: 16, left: 20, zIndex: 1000, pointerEvents: "none" }}>
        <div style={{ fontSize: 10, color: "#00e5c0", letterSpacing: 2, fontFamily: "'DM Mono',monospace", textShadow: "0 2px 4px rgba(0,0,0,0.8)" }}>AEROSENSE-AI · LIVE INTERACTIVE MAP</div>
        <div style={{ fontSize: 24, fontWeight: 800, color: "#e0e8f0", textShadow: "0 2px 4px rgba(0,0,0,0.8)" }}>{originCode} <span style={{ color: "#aaa" }}>→</span> {destCode}</div>
      </div>
      <div style={{ position: "absolute", bottom: 16, right: 20, textAlign: "right", zIndex: 1000, pointerEvents: "none" }}>
        <div style={{ fontSize: 9, color: "#aaa", letterSpacing: 2, fontFamily: "'DM Mono',monospace", marginBottom: 4 }}>GLOBAL CARTOGRAPHY MODULE</div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-end", padding: "4px 8px", background: "rgba(0,0,0,0.7)", borderRadius: "4px", border: "1px solid rgba(0,229,192,0.2)" }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#00e5c0", animation: "pulse 2s infinite" }} />
          <div style={{ fontSize: 11, color: "#e0e8f0", fontFamily: "'DM Mono',monospace" }}>INTERACTIVE</div>
        </div>
      </div>
    </div>
  );
...
export default function AviationAI() {
  const [page, setPage] = useState("login"); // "login" | "select" | "dashboard" | "history" | "profile" | "password" | "activity" | "admin-control"
  const [flights, setFlights] = useState(() => {
    const saved = localStorage.getItem('aerosense_flights');
    return saved ? JSON.parse(saved) : FLIGHT_DETAILS;
  });
  const [aircraftRequests, setAircraftRequests] = useState(() => {
    const saved = localStorage.getItem('aerosense_requests');
  ...
  useEffect(() => {
    localStorage.setItem('aerosense_flights', JSON.stringify(flights));
  }, [flights]);

  useEffect(() => {
    localStorage.setItem('aerosense_requests', JSON.stringify(aircraftRequests));
  }, [aircraftRequests]);
    ...
    <>
      ...
      {page === "select" && (
        <FlightSelectPage
          ...
          onViewAdminControl={() => currentUser?.isAdmin && setPage("admin-control")}
        />
      )}
      {page === "dashboard" && (
        <Dashboard
      ...
      {page === "ai-schedule" && currentUser?.isAdmin && (
        <AiSchedulePage onBack={() => setPage("select")} currentUser={currentUser} />
      )}
      {page === "password" && (
        <PasswordChangePage
          currentUser={currentUser}
      ...
      {page === "admin-control" && currentUser?.isAdmin && (
        <AdminControlPage
          currentUser={currentUser}
          flights={flights}
          setFlights={setFlights}
          aircraftRequests={aircraftRequests}
          setAircraftRequests={setAircraftRequests}
          onBack={() => setPage("select")}
        />
      )}
...
function Dashboard({ flight, onBack }) {
  ...
  const tabs = [
    ...
    { id: "simulation", label: "SIMULATION", badge: "AI" },
  ];
      ...
      <div style={{ padding: isMobile ? "32px 24px" : "24px 28px", animation: "fadeIn .3s ease", paddingBottom: isMobile ? 80 : 60 }}>
        ...
        {tab === "simulation" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn .4s ease" }}>
            {/* Simulation Header: Origin to Destination */}
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", overflow: "hidden", flexWrap: "wrap", gap: 20 }}>
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(90deg, rgba(0,229,192,0.05), rgba(0,118,255,0.05))", pointerEvents: "none" }} />

              <div style={{ textAlign: "center", zIndex: 1, minWidth: 80 }}>
                <div style={{ fontSize: 36, fontWeight: 800, color: "#e0e8f0" }}>{flight.route.split('→')[0]}</div>
                <div style={{ fontSize: 10, color: "#888", letterSpacing: 2, fontFamily: "'DM Mono',monospace", marginTop: 4 }}>TAKEOFF</div>
              </div>

              <div style={{ flex: 1, display: "flex", alignItems: "center", padding: isMobile ? "0 10px" : "0 40px", zIndex: 1, minWidth: 200 }}>
                <div style={{ width: 12, height: 12, borderRadius: "50%", background: "rgba(0,229,192,0.2)", border: "2px solid #00e5c0", flexShrink: 0 }} />
                <div style={{ flex: 1, height: 2, background: "repeating-linear-gradient(90deg, #555 0, #555 4px, transparent 4px, transparent 8px)", margin: "0 10px", position: "relative" }}>
                  <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", fontSize: 24, color: "#00e5c0", textShadow: "0 0 10px rgba(0,229,192,0.5)" }}>✈</div>
                </div>
                <div style={{ width: 12, height: 12, borderRadius: "50%", background: "rgba(0,118,255,0.2)", border: "2px solid #0076ff", flexShrink: 0 }} />
              </div>

              <div style={{ textAlign: "center", zIndex: 1, minWidth: 80 }}>
                <div style={{ fontSize: 36, fontWeight: 800, color: "#e0e8f0" }}>{flight.route.split('→')[1]}</div>
                <div style={{ fontSize: 10, color: "#888", letterSpacing: 2, fontFamily: "'DM Mono',monospace", marginTop: 4 }}>DESTINATION</div>
              </div>
            </div>

            {/* 3D Map Visualization */}
            <SimulationGlobe route={flight.route} />

            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 2fr", gap: 16 }}>
              {/* AI Requirements */}
              <div style={{ background: "rgba(0,229,192,0.03)", border: "1px solid rgba(0,229,192,0.15)", borderRadius: 10, padding: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                  <div style={{ fontSize: 18 }}>🤖</div>
                  <div style={{ fontSize: 10, color: "#00e5c0", letterSpacing: 2, fontFamily: "'DM Mono',monospace", fontWeight: 700 }}>AI PRE-FLIGHT REQUIREMENTS</div>
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: 14, borderLeft: "3px solid #ff3b5c" }}>
                    <div style={{ fontSize: 9, color: "#ff3b5c", letterSpacing: 1.5, marginBottom: 4, fontFamily: "'DM Mono',monospace" }}>FUEL REQUIREMENT</div>
                    <div style={{ fontSize: 11, color: "#bbb", lineHeight: 1.6 }}>Fuel tank must be provisioned for <span style={{ color: "#e0e8f0", fontWeight: 700 }}>+45 min</span> contingency auxiliary reserve due to expected holding patterns over Central Europe airspace.</div>
                  </div>
                  <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: 14, borderLeft: "3px solid #f5a623" }}>
                    <div style={{ fontSize: 9, color: "#f5a623", letterSpacing: 1.5, marginBottom: 4, fontFamily: "'DM Mono',monospace" }}>WEIGHT & BALANCE</div>
                    <div style={{ fontSize: 11, color: "#bbb", lineHeight: 1.6 }}>Shift cargo loading to aft compartments (-2% MAC) to optimize cruise angle of attack, saving estimated 1.2t fuel.</div>
                  </div>
                  <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: 14, borderLeft: "3px solid #00e5c0" }}>
                    <div style={{ fontSize: 9, color: "#00e5c0", letterSpacing: 1.5, marginBottom: 4, fontFamily: "'DM Mono',monospace" }}>THRUST SETTING</div>
                    <div style={{ fontSize: 11, color: "#bbb", lineHeight: 1.6 }}>Use TO-2 derated thrust for departure. Runway length and temperature (ISA +4°C) permit lower engine stress while maintaining safety margins.</div>
                  </div>
                </div>
              </div>

              {/* Start to End Analysis */}
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 20, display: "flex", flexDirection: "column" }}>
                <div style={{ fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 20, fontFamily: "'DM Mono',monospace", fontWeight: 700 }}>START-TO-END TELEMETRY ANALYSIS</div>

                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16, overflowX: "auto" }}>
                  <div style={{ minWidth: isMobile ? 500 : "auto", display: "flex", flexDirection: "column", gap: 16 }}>
                    {[
                      { phase: "DEPARTURE", weather: "Clear / 12kt Crosswind", speed: "V2 + 15kts", flow: "Laminar", alt: "0 - 10,000 ft", color: "#00e5c0", icon: "🛫" },
                      { phase: "CRUISE (INITIAL)", weather: "Light Chop", speed: "Mach 0.83", flow: "Turbulent boundary", alt: "FL320", color: "#0076ff", icon: "✈️" },
                      { phase: "CRUISE (OPT)", weather: "Tailwind + 40kt", speed: "Mach 0.84", flow: "Stable", alt: "FL360 / FL380", color: "#7600ff", icon: "🌤️" },
                      { phase: "DESCENT", weather: "Overcast / Rain squalls", speed: "280kts → 250kts", flow: "Moderate thermals", alt: "FL200 - 3,000 ft", color: "#f5a623", icon: "🛬" },
                    ].map((p, i) => (
                      <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 3fr 2fr 2fr", gap: 12, padding: "12px 16px", background: "rgba(0,0,0,0.2)", borderRadius: 8, alignItems: "center", borderLeft: `2px solid ${p.color} ` }}>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <span style={{ fontSize: 16 }}>{p.icon}</span>
                          <div>
                            <div style={{ fontSize: 9, color: p.color, letterSpacing: 1, fontFamily: "'DM Mono',monospace" }}>PHASE</div>
                            <div style={{ fontSize: 11, fontWeight: 700, marginTop: 2, whiteSpace: "nowrap" }}>{p.phase}</div>
                          </div>
                        </div>

                        <div>
                          <div style={{ fontSize: 9, color: "#666", letterSpacing: 1, fontFamily: "'DM Mono',monospace" }}>WEATHER / AIRFLOW</div>
                          <div style={{ fontSize: 11, color: "#bbb", marginTop: 2 }}>{p.weather} <span style={{ color: "#555" }}>•</span> {p.flow}</div>
                        </div>

                        <div>
                          <div style={{ fontSize: 9, color: "#666", letterSpacing: 1, fontFamily: "'DM Mono',monospace" }}>SPEED TARGET</div>
                          <div style={{ fontSize: 11, color: "#00e5c0", marginTop: 2, fontFamily: "'DM Mono',monospace" }}>{p.speed}</div>
                        </div>

                        <div>
                          <div style={{ fontSize: 9, color: "#666", letterSpacing: 1, fontFamily: "'DM Mono',monospace" }}>ALTITUDE</div>
                          <div style={{ fontSize: 11, color: "#e0e8f0", marginTop: 2, fontFamily: "'DM Mono',monospace" }}>{p.alt}</div>
                        </div>
                      </div>
                    ))}
                  </div>
